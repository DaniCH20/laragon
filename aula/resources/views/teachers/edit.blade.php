...
@section('content')
    <div class="container">
        ...
        <form class="mt-2" name="create_platform" action="{{ route('teachers.update', $teacher) }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            <div class="form-group mb-3">
                <label for="dni" class="form-label">Nombre</label>
                <input type="text" class="form-control" id="nombre" name="nombre"
                    value="{{ $teacher->nombreApellido }}" />
            </div>
            <div class="form-group mb-3">
                <label for="profesion" class="form-label">Profesion</label>
                <input type="txt" class="form-control" id="profesion" name="profesion"
